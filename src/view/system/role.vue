<template>
  <div>
    <div style="margin-top:20px">
      <div style="padding-bottom:10px;"><Button type="primary" icon="md-person-add" @click="roleModal()">新增角色</Button></div>
      <table class="table padding-10 hover hide-checkbox-label">
        ...
        <tr class="center" v-for="(role,i) in roles" :key="i">
          ...
          <td class="center">
            <ButtonGroup>
              <Button type="primary" ghost style="width:100px;" @click="setUserRoleModal(role)">设置用户</Button>
    ...
    <Modal v-model="roleModalShow" :loading="saveLoading" :title="roleFormData.id?'编辑角色':'新增角色'" @on-ok="saveRole">
      <div>
        <Form ref="formValidate" :model="roleFormData" :rules="formValidate" :label-width="100" style="padding-right:10%">
    ...
    <Modal v-model="userModalShow" :title="'设置用户 - '+userFromData.role.name" width="480" footer-hide>
      <div v-if="userFromData.fromUsers" style="margin:5px auto">
        <Transfer
                ...
                :operations="['移除','增加']"
                filterable
                @on-change="setChange" class="transfer-set"></Transfer>
      </div>
    </Modal>
      ...
      <div class="menu-list scrollbar">
        <CellGroup>
          <template v-for="menu in menuFromData.menuList">
            <Cell class="menu-list-dir"><span style="font-weight:bold">{{menu.name}}</span><Icon :type="menu.logo" slot="icon" size="16" style="vertical-align: -1px" />
              <i-switch v-model="menu.switch" @on-change="changeMenu(menu)" slot="extra" />
            </Cell>
            <template v-for="item in menu.children">
            <Cell :title="item.name" class="menu-list-menu" :style="item.children.length>0?'border:none':''">
              <i-switch v-model="item.switch" @on-change="changeMenu(item)" slot="extra" />
            </Cell>
            <div v-if="item.children.length>0" class="menu-list-button">
              <span v-for="(element,iii) in item.children" :key="'element'+iii" class="item-button">
                {{element.name}} <i-switch size="small" v-model="element.switch" @on-change="changeMenu(element)" />
              </span>
            </div>
            </template>
  ...
  export default {
    data(){
      return {
        ...
        formValidate: {
          ...
          code: [
            {type:'string',required: true, message: '请填写角色代码', trigger: 'blur'}
          ]
        },
        roleFormData:{
          id:null,
    ...
    methods:{
      ...
      saveRoleAction(){
        ...
        var service=(roleData.id)?$RoleService.update(roleData):$RoleService.insert(roleData)
        service.then(res=>{
          if (res.code===1){
            this.$Message.success(roleData.id?'角色更新成功！':'角色新增成功！')
      ...
      changeMenu(obj){
        if (obj.switch==false&&obj.relation){
          ...
          $RoleMenuService.insert({roleId:this.menuFromData.role.id.toString(),menuId:obj.id.toString()}).then(res=>{
            if (res.code===1){
      ...
      setMenuModal(role){
        ...
        Promise.all(request).then(res=>{
          let menuList=this.matchMenuList(res[0].data.records,res[1].data.records)
      ...
      matchMenuList(source,target){
        source.map(item=>{
          let findRrelation=target.find(ele=>ele.menuId.toString()===item.id.toString())
          if (findRrelation) {
      ...
      setUserRoleModal(role){
        ...
        Promise.all(request).then(res=>{
          ...
          res[0].data.records.map(item=>{
            let findRrelation=res[1].data.records.find(ele=>ele.userId.toString()===item.id.toString())
            fromUsers.push({key:item.id.toString(), label:item.username,relationId:findRrelation?findRrelation.id.toString():null})
          })
          let userRole=res[1].data.records.map(item=>item.userId.toString())
          var targetUsers=fromUsers.filter(item=>userRole.includes(item.key)).map(item=>item.key)
      ...
      setChange(targetKeys, direction, moveKeys){
        ...
        if (direction=='right'){
          moveKeys.forEach(key=>{
            $UserRoleService.insert({roleId:this.userFromData.role.id.toString(),userId:key}).then(res=>{
              if (res.code===1){
          ...
          let relations=this.userFromData.fromUsers.filter(item=>moveKeys.includes(item.key))
...
<style lang="less">
  .transfer-set .ivu-transfer-list {height:280px}
</style>
<style lang="less" scoped>
  .menu-list {
    ...
    &-menu {
      margin-left:20px;border-bottom:1px dotted #F2F2F2
    }
    &-button {
      padding:5px 15px 10px 15px;
      margin-left:20px;border-bottom:1px dotted #F2F2F2;
      .item-button {
        display:inline-block;border:1px solid #CCC;padding:4px 8px;border-radius:2px;margin:2px 5px 2px 0;
      }
    }
  }
</style>
