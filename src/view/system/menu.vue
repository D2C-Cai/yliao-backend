<template>
  <div>
    <Row :gutter="30">
      <Col :xxl="8" :xl="10" :lg="12" :md="12">
        <Card dis-hover>
          <p slot="title">
            <Icon type="md-grid" />
            菜单管理
          </p>
          <a @click="addNew()" slot="extra">
            新增
          </a>
      ...
      <Col :xxl="8" :xl="10" :lg="12" :md="12">
        <Card dis-hover>
          ...
          <Form ref="formValidate" :rules="formValidate" :model="formData" :label-width="80" style="padding-right:10%">
            <FormItem>
            ...
            <FormItem label="菜单图标">
              <Input v-model="formData.logo" icon="md-help-circle" style="width:90%" /> <Icon :type="formData.logo" size="30" />
            </FormItem>
            <FormItem label="菜单排序">
            ...
            <FormItem style="text-align:right">
              <div style="display:flex;justify-content: space-between">
                <div>
                  <a @click="remove(formData)" slot="extra" style="color:#FF0000" v-if="formData.children && formData.children.length==0"> 删除 </a>
                </div>
                <div>
                  <Button style="margin-right:15px" @click="addNew(formData)" v-if="formData.id && formData.type!='BUTTON'">新增下级</Button>
  ...
  export default {
    data(){
      return {
        ...
        formValidate: {
          name: [
            {type:'string',required: true, message: '请填写菜单名称', trigger: 'blur'}
          ],
          path: [
            {type:'string',required: true, message: '请填写请求路径', trigger: 'blur'}
          ],
          route: [
            {type:'string',required: true, message: '请填写前端路由', trigger: 'blur'}
          ],
          assembly: [
            {type:'string',required: true, message: '请填写前端组件', trigger: 'blur'}
          ]
        }
      }
    },
      ...
      saveMenu(){
        this.$refs['formValidate'].validate((valid) => {
          if (valid) {
            ...
            let service=formData.id?$MenuService.update(formData):$MenuService.insert(formData)
            service.then(res=>{
              if (res.code===1){
                this.$Message.success(formData.id?'菜单编辑成功':'菜单新增成功')
                if (!formData.id){
                  var result={...res.data}
                  result.parentId=findParent(this.menuList,result.parentId.toString(),[])
...
.menu-tree {position:relative;max-height:69vh;height:100%;overflow:auto;}
</style>
