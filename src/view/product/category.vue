  ...
  <div>
    ...
    <table class="table padding-10 hover">
      <tr class="header">
        ...
        <td class="center">是否首页<Tooltip content="首页最多展示8个类目"><Icon type="md-help-circle" size="18" color="#2d8cf0" style="vertical-align:-3px" /></Tooltip></td>
        <td class="right">操作</td>
      </tr>
      <tbody>
      <template v-for="(item,i) in categoryList">
      <tr>
        <td><strong>{{item.name}}</strong>  <span style="color:#CCC;font-size:12px;">({{item.id.toString()}})</span></td>
        <td class="center"><img :src="imgUrl+item.pic+'!/fh/60'" /></td>
        <td class="center">{{item.sort}}</td>
        <td class="center"><img v-if="item.icon" :src="imgUrl+item.icon" /></td>
        <td class="center"><i-switch v-model="item.display" :true-value="1" :false-value="0" @on-change="changeDisplay(item)"></i-switch></td>
        <td class="right"><ButtonGroup>
          ...
          <Button icon="md-trash" type="error" @click="deleteCategory(item)"  :disabled="item.children && item.children.length>0">删除</Button>
        </ButtonGroup></td>
      </tr>
      <template v-if="item.children && item.children.length>0">
      <tr v-for="(element,ii) in item.children" :key="i+'-'+ii">
        <td><span style="color:#CCC">{{item.children.length==ii+1?'└':'├'}}──</span> <strong>{{element.name}}</strong> <span style="color:#CCC;font-size:12px;">(ID:{{element.id.toString()}})</span></td>
        <td class="center"><img :src="imgUrl+element.pic+'!/fh/60'" /></td>
        <td class="center">{{element.sort}}</td>
        <td class="center"><img v-if="element.icon" :src="imgUrl+element.icon" /></td>
        <td class="center"><i-switch v-model="element.display" :true-value="1" :false-value="0" @on-change="changeDisplay(item)"></i-switch></td>
        <td class="right"><ButtonGroup>
    ...
    <Modal :title="categoryModalTitle" :loading="saveLoading" v-model="categoryModalShow" @on-ok="saveCategory">
        ...
        <Form ref="formValidate" :model="formData" :rules="formValidate" :label-width="80">
          <FormItem label="上级分类">
            <Select v-model="formData.parentId" clearable>
              ...
              <Option v-for="category in categoryList" :key="category.id.toString()" :value="category.id.toString()">{{category.name}}</Option>
            </Select>
          </FormItem>
          ...
          <FormItem label="分类图片" prop="pic">
            <upload-image-upyun :img-list="formData.pic" size="normal" :maxLength="num" @on-success="uploadPicSuccess" @on-remove="uploadPicRemove"></upload-image-upyun>
          </FormItem>
          <FormItem label="首页ICON">
            <upload-image-upyun :img-list="formData.icon" size="small" :maxLength="num" @on-success="uploadIconSuccess" @on-remove="uploadIconRemove"></upload-image-upyun>
          </FormItem>
          <FormItem label="首页显示">
            <i-switch v-model="formData.display" :true-value="1" :false-value="0"></i-switch>
          </FormItem>
          <FormItem label="显示排序">
            <Input type="number" v-model="formData.sort" style="width:60px" clearable></Input>
          </FormItem>
        </Form>
      </div>
    </Modal>
...
<script>
  ...
  export default {
    data(){
      return {
        ...
        formValidate: {
          ...
          pic:[
            {type:'array',min: 1,required: true, message: '请上传图片', trigger: 'blur'}
          ],
        }
      }
    },
    ...
    methods:{
      changeDisplay(item){
        $ProductCategoryService.update({id:item.id.toString(),display:item.display}).then(res=> {
          if (res.code===1){
          ...
          $ProductService.select({categoryId:item.id.toString()}).then(res=>{
            if (res.code==1&& res.data.records.length>0){
              ...
              this.$Modal.confirm({
                ...
                content:'确定要删除该分类吗？',
                onOk:()=>{
                  $ProductCategoryService.delete({ids:[item.id.toString()]}).then(res=>{
                    if (res.code===1){
                      this.getList()
                    }
                  })
      ...
      editCategory(item){
        ...
        if(item){
          var formData={...item}
          formData.parentId=formData.parentId?formData.parentId.toString():0
