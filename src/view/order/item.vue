          ...
          <table class="table order-table hover-shadow hide-checkbox-label padding-10" v-for="item in orderItems.list" :key="item.id.toString()">
            ...
            <tr>
              <td style="line-height:26px">
                ...
                <div>订单编号：<Tooltip content="点击复制" placement="top" v-clipboard:copy="item.orderSn" v-clipboard:success="copySuccess" style="cursor:pointer"><strong>{{item.orderSn}}</strong></Tooltip> &nbsp;&nbsp;&nbsp; 创建时间：{{item.createDate}}</div>
              </td>
              <td>
  ...
  export default {
    ...
    methods:{
      getOrderItemList(p=1,size=20){
        ...
        $OrderItemService.select(params).then(res=>{
          if (res.code===1){
            this.orderItems={
