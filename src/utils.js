...
const dealUserInfo = (info) => {
    ...
    let userInfo = {
        ...
        amount: info.accountInfo.oauthAmount,
        amountCreateTime: info.accountInfo.createDate.replace(/-/g, '/'),
        amountExpireTime: info.accountInfo.deadline.replace(/-/g, '/'),
        p2pId: info.accountInfo.p2pId.toString()
    }
...
const upyunSignature = () => {
    ...
    let signature = md5(policy + '&yEOmoIqs+ZsKd4dFHaxGTfyQzNo=')
    return {
        policy: policy,
        signature: signature
    }
...
export {LocalStorage, dealUserInfo, upyunSignature, listToTree, discountCaculate, randomString, findParent}
